...
async function robot(){
    ...
    async function authenticateWithOAuth(){
        ...
        async function waitForGoogleCallback(webServer){
            return new Promise((resolve, reject) => {
                ...
                webServer.app.get('/oauthcallback', (req, res) => {
                    const authCode = req.query.code;
                    console.log(`> [youtube-robot] User consent: ${authCode}`)

                    res.send(`
                        <h1>Thank you!</h1>
                        <p>Now close this tab</p>
                    `)
                    resolve(authCode);
                })
            })
        ...
        async function requestGoogleForAcessToken(OAuthClient, authorizationToken){
            return new Promise((resolve, reject) => {
                OAuthClient.getToken(authorizationToken,( error,tokens ) => {
                    ...
                    console.log('> [youtube-robot] Acess token received:')
                    console.log(tokens)
    ...
    async function uploadVideo(content){
        ...
        const videoFileSize = fs.statSync(videoFilePath);
        ...
        function onUploadProgress(event) {
            if(!event.bytesRead) {
                console.log(`> [youtube-robot] Without progress`) 
                return;
            }
